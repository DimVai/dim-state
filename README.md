...
*As an alternative* (not recommended for now), you can import the `dim-state-module.js` (located in folder 4) *as a module*, but its functionality is not tested or supported fully:
```JavaScript
    import State from './dim-state-module.js';
```
...
### **2st step:**
Optionally (recommended), run the following command **once**, on ```window.load``` for example, to make your life better later:
```JavaScript
